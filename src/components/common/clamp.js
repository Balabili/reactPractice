//原作https://github.com/zoltantothcom/react-clamp-lines
    ...
    while (this.start <= this.end) {
      ...
      if (this.middle === this.original.length) {
        if (this.element.clientHeight >= maxHeight) {
          this.element.innerText = this.original.slice(0, this.middle - 3) + this.getEllipsis();
          this.setState({ text: this.original.slice(0, this.middle - 3) + this.getEllipsis() });
        } else {
          this.setState({
            text: this.original,
            noClamp: true,
